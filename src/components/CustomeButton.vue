<template>
  <div>
    ...
    <el-dialog :title="options.Name" :visible.sync="dialogVisible" size="tiny" :before-close="handleClose">
...
<script>
  ...
  import { getByKey, getUrlSearch } from '@/config/common';
  export default {
    name: 'customeButton',
    props:['options','scope'],
    data () {
    ...
    methods: {
      renderDialog() {
        ...
        switch (operateCode) {
          ...
          case "Print":
            ...
            return false;
          case "UpPwd":
            post(this.apiUrlAdmin + operatePath,{userId: userId}).then(res => {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            post(this.apiUrlAdmin + '/User/UserPwdUpdate',{userId: this.dialogData.Id, old: this.dialogData.Password, pwd: this.updatePass.newPass}).then(res => {
              this.$store.dispatch('setAlertOption',{
                show: true,
                type:'success',
                title:res.Message
